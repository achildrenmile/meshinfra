    ...
    def _on_disconnect(
        ...
        if rc != 0:
            ...
            if self._running and not self._reconnecting:
                ...
                # This callback runs on the paho network thread, which has no
                # running event loop. asyncio.create_task() therefore raises
                # RuntimeError("no running event loop"), the exception is
                # swallowed by paho, and the MQTT side stays down forever while
                # the process keeps running and looks healthy.
                # Schedule on the stored loop instead, the same way
                # _forward_command_to_meshcore already does.
                loop = self._event_loop
                if loop is not None and not loop.is_closed():
                    asyncio.run_coroutine_threadsafe(
                        self._recover_connection(), loop
                    )
                else:
                    self.logger.error(
                        "No usable event loop for MQTT recovery - "
                        "connection will stay down"
                    )
        else:
