...
class MeshCoreWorker:
    ...
    async def _perform_health_check(self) -> bool:
        ...
        try:
            ...
            # dcsetup-Patch (2026-09-09): Upstream-Default war 180s - auf einem
            # ruhigen Mesh-Kanal ohne Traffic (ham-mesh, sporadische Nutzung)
            # ist das eine reine Inaktivitaets-Heuristik, KEIN echter
            # Verbindungstest ("healthy" ist an dieser Stelle bereits true).
            # Ergebnis: alle paar Minuten ein unnoetiger Full-Reconnect, der
            # fuer ca. 1s die TCP-Session zum Companion-Radio killt - genau in
            # diesem Fenster ankommende Kanalnachrichten (z.B. !frag/!ping)
            # gehen verloren, ohne dass irgendwo ein Fehler/Drop gezaehlt wird
            # (siehe docs/mesh-broker-map.md, "Vorfall meshbot stumm" und
            # dcsetup-Session vom 2026-09-09). 1800s (30 Min) laesst dem
            # eigentlichen `_check_connection_health()` genug Raum, echte
            # Ausfaelle weiterhin sofort zu erkennen (der bleibt unveraendert),
            # ohne bei normaler Funkstille faelschlich zu reconnecten.
            if healthy and self._is_stale(timeout_seconds=1800):
